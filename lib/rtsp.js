...
function Client(volume, password, audioOut) {
...
Client.prototype.setVolume = function(volume, callback) {
  // console.log('setVolume', this.status, this.status == PLAYING)
  // set volume even if not ready, as this is called after 'RECORD'
  // to sync volume as playback begins
  this.volume = volume;

  if(this.status !== PLAYING) {
    this.commandQueue.push({ command: SETVOLUME, callback: callback })
    return;
  }
...
Client.prototype.setTrackInfo = function(name, artist, album, callback) {
  // console.log('setTrackInfo', this.status, this.status == PLAYING)
  ...
  if(this.status !== PLAYING) {
    this.commandQueue.push({ command: SETDAAP, callback: callback })
    return;
  }
...
Client.prototype.setTrackLength = function(length, callback) {
  // console.log('setTrackLength', this.status, this.status == PLAYING)

  this.length = length;

  if(this.status !== PLAYING) {
    this.commandQueue.push({ command: SETLENGTH, callback: callback })
    return;
  }

  this.status = SETLENGTH;
  this.callback = callback;
  this.sendNextRequest();
};

Client.prototype.setArtwork = function(art, contentType, callback) {
  // console.log('setArtwork', this.status, this.status == PLAYING)
  ...
  if(this.status !== PLAYING) {
    this.commandQueue.push({ command: SETART, callback: callback })
    return;
  }
...
Client.prototype.setProgress = function(progress, callback) {
  // console.log('setProgress', this.status, this.status == PLAYING)

  this.progress = progress;

  if(this.status !== PLAYING) {
    this.commandQueue.push({ command: SETPROGRESS, callback: callback })
    return;
  }
...
function parseResponse(blob) {
  ...
  // Some clients seem to send messages out of order
  // this is a hack to ignore a message before the response
  // code
  if (lines[0].includes('Audio-Latency')) {
    console.log('parseResponse: unshifting Audio-Latency')
    lines.shift()
  }
  ...
  // console.log('parseResponse header', response.headers)
  return response;
}
...
Client.prototype.sendNextRequest = function(di) {
  ...
  case SETLENGTH:
    var length = this.daapEncodeUInt32BE('astm', this.length);
    var daapInfo = this.daapEncodeList('mlit', length);
  ...
  case PLAYING:
    ...
    var next = this.commandQueue.shift()
    this.status = next.command
    this.callback = next.callback
    // console.log('iterating queue', lastStatus, this.status)
    this.sendNextRequest()
...
Client.prototype.daapEncodeUInt32BE = function(field, value) {
  var buf = new Buffer(field.length + 4 + 4);
  buf.write(field, 0, field.length, 'ascii');
  buf.writeUInt32BE(4, field.length);
  buf.writeUInt32BE(value, field.length + 4);
  return buf;
};
